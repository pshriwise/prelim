...
\begin{center}
  \begin{tikzpicture}
    ...
    \visible<2>{\node (CAD L) [auto,outer sep=0pt,below = 0.1 cm of CAD] {\small geometric design};
      \node (fng cad)[auto,below = 0.1cm of CAD L,outer sep=0pt] {\includegraphics[scale=0.07]{fng_cad.png}};
    ...
    \pause %%%%
    \node [auto,below = 0.1cm of TOOLKIT L,outer sep=0pt] {\includegraphics[scale=0.07]{fng_results.png}};
...
\item Quad-tree
  ...
  \end{itemize}
\item compensates by using advanced vector extentions (AVX) and single instruction multiple data (SIMD) instruction sets to check many nodes at once.
\end{itemize}
