...
\institute{University of Wisconsin-Madison, 1500 Engineering Dr,
  Madison, WI 53706, wilsonp@engr.wisc.edu}
...
summarized here for context.  A user imports a geometric description
generated with a solid modeling software
packages and into Cubit \cite{cubit} for
pre-processing steps including assignment of materials and boundary
conditions, and imprinting and merging.  This geometry file is used to
...
time-consuming translation \cite{fusion3d} to native geometric representations of the
...
language paradigms to define a formal interface that can be implemented
...
CERN, ATLAS, LHC, etc, however it is widely used for detector simulations and 
...
to next surface and the volume on the other side of the next
surface. The user simply takes the \C++ routines that represent the
geometry, compiles these to object files, links against a limited subset
of the Geant4 libraries and then links against the FLUKA libraries,
...
\begin{description}
  ...
  volume that position belongs to.}
...
detailed material descriptions, but does handle the material assignments.
...
equivalent. 
%% The models were built using native MCNP geometries,
%% converted to CAD using mcnp2cad for FluDAG, and converted to Fluka by
%% importing the MCNP geometry using the Flair \cite{flair} interface. 
Each test
...
beam of neutrons start particles at an x coordinate of 0.0 cm. Overriding 
the volume of each scoring region to be 1.0 cm$^3$ should result in a track length score of 
...
As shown in Table \ref{usrbdx_crossing_a} the results are as expected. 
...
performed. In order to demonstrate the USRBDX score (directed particle current) 
between two volumes, a
model was created with a heavy water sphere of radius 50.0 cm, 
with particles being killed once they leave the sphere. An
...
neutron flux crossing the sphere in both angle (60 bins) and energy (260 bins).
...
Figure \ref{fig:mat_usrbdx} shows the ratio of the
...
the ratio of results for the first angular bin corresponding to $(0,0.209)$ 
steradians, and the lower part of Figure \ref{fig:mat_usrbdx}
shows the ratio of results for the $20^{th}$ angular bin corresponding to 
$(3.979, 4.189)$ steradians. We see that for both figures, the ratio of the
...
defining a water region. A disk source located at x=0.001 of radius XX cm provides a 
parallel beam of 1 GeV photons.

\begin{figure}%[h!]
	\begin{center}
		\epsfig{file=figs/mesh_phot.ps,width=.5\columnwidth,angle=-90}
		\epsfig{file=figs/mesh_phot_zoom.ps,width=.5\columnwidth,angle=-90}
...
bounds defined by $\pm 3\sigma$. The large deviation away from the average 
(as well as the large error bounds at this location) is due to 
...
that in a model composed entirely of vacuum no particles were lost and the flux was well-behaved.
...
interface, with different implementations included with the base
...
magnetic fields, and incorporate the ability to tally on unstructured
tetrahedral meshes.
