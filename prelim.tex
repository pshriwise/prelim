...
%% Implicit surfaces can be a boolean combination of CSG objects, but are able to represnt more complex manifolds as well. CSG objects are relatively straightforward to compute In visualization, implicit surfaces are used to generate triangle meshes for rasterization or rendering on Graphics Processing Units (GPUs)\cite{Set96} and can also be constructed around arbitrarly triangle meshes\cite{Lev98}. When using these implicit surface representations in implementation, the implicit surface's derived signed distance field 
