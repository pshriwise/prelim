...
Using a visitor pattern in the BVH, each oriented bounding box was converted into a MOAB hex element and saved on an entirely different mesh. Each hex element was additionally tagged with it's depth in the tree. This program then outputs a mesh which can be used to visualize the BVH level by level on top of the geometric mesh. At lower depths in the BVH, only bounding boxes in the high valence region are present. This is expecting in that it will take more bounding boxes and therefore a deeper tree in that region in order to resolve the larger number of triangles. What was intereseting to note in this region was that the bounding boxes of the high valence surface would often contain some portion of the high valence region's triangles as well as another triangles in the high valence surface. MOAB's BVH building algorithm employs the median splitting method with some parameters indicating whether a node split is considered worthwhile. These parameters have to do with how closely the median planes can split the entities contained by the parent bounding box. Specifically, one parameter is set such that if over 95\% of the entities end up in one of the child boxes, then the node will not be split and thus becomes a leaf node. In the high valence case, it is highly unlikely the aforementioned splitting criterion will be met and so there will exist leaves with a large number of triangles in them. Not only is this a problem in the sense that it implies a large number of triangle checks will be necessary if this leaf node is hit, but if this leaf node also contains triangles outside the high valence region with large area (as we tend to see in this model) the leaf becomes much more likely to be traversed, even though the chances that the ray is going to hit a triangle in the high valence region is relatively small. As the high valence region grows smaller relavitve to the surface size, this problem is only exacerbated in that the high valence region triangles become more closely packed together and less likely to be split by a median plane and the triangles exterior to the high valence region become larger, causing the leaf node bounding box to be hit even more often. In this way the splitting algorithm not only deals with the high valence region poorly, but it may also create an artificially high cross-section of the high-valence region.


(define splitting ration above, first equation!!! omgomgomg)
(mention triangle density shock in here maybe?)

Fortunately MOAB allows one to have control over the parameters used to govern the BVH building algorithm. By allowing for splits 
