...
As described in Section \ref{emdag}, a SIMD-enabled ray tracing kernal, Embree, has already been applied within the DAGMC framework with great success. Unfortunately DAGMC's robustness was lost due to a fundamental difference in the precision used to store geometric primitives and intersect rays. It is believed that this problem can be remedied in a straightforward manner by altering these datatypes from single to double floating point precision to match those of the underlying physics codes DAGMC supports and adjusting the underlying algorithms accordingly. A substantial reduction in performance due to this change in floating point calculations is not expected however as the construction of the underlying hierarchy used to isolate triangles for intersection can remain in single precision if built appropriately.

There are two ways to accomplish the above goal of applying SIMD-enabled ray tracing robustly within DAGMC. The first is to alter Embree to meet the requirements of higher precision primitives and expanded single precision bounding boxes and link it to the MOAB primitives as was done in previous work. An alternative approach, and perhaps a more desirable one, is to design a kernal with many of the features that exist in Embree but that couples more cleanly with DAGMC's underlying mesh database, MOAB. MOAB's design does not easily support such a kernel directly, but its ability to store data in a contiguous manner and provide direct access to that data would meet the necessary basic requirements for a SIMD-enabled ray tracing kernel that is more catered toward the current needs of the scientific community.

Despite this loss in robustness, higher nary bounding volume hierarchies designed for vectorized traversal align well with the priorities of acceleration datastructures used in large, complicated models. 
...
%% This process has allowed for successful transport runs of complex models of the International Thermonuclear Experimental Reactor (ITER) for runs upward of 1E8 histories with zero lost particles.\cite{smith_thesis_2011}
