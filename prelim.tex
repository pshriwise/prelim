...
\subsection{Implicit Surface Representations}%%Status: Pending%%
...
Implicit surfaces can be composed of some set of CSG objects, but are able to represnt more complex manifolds as well. Implicit surfaces are commonly used to generate triangle meshes for visualization via rasterization or rendering on Graphics Processing Units (GPUs). This is accomplished via a fast marching method \cite{Set96} for determining an appropriate discretization of the surface. Conversely, implicit surfaces can be reconstructed from triangle meshes using a variety of methods, such as the moving least squares approach \cite{Lev98}.


Implementation of implicit surfaces commonly involvses re-sampling of the function for different time steps as simulations are computed. 
